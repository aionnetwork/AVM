...
public enum ABIToken {
     ...
     */
    public static final Map<Class<?>, ABIToken> STANDARD_LEAF_TYPE_MAP = Stream.of(ABIToken.values()).filter(token -> token.isLeafType).collect(Collectors.toMap(token -> token.standardType, token -> token));
